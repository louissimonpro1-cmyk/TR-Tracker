...
import { resolveSymbol, dailyHistory, intradayHistory, hourlyHistory, getFx } from "./yahoo.mjs";
...
export function makePricer(asset, market) {
  ...
  return {
    ...
    eurAt, adjEurAt, intraEurAt, hourlyEurAt,
...
const OWN_PERF_EMPTY = { d1: null, w1: null, m1: null, m6: null, y1: null, y3: null, y5: null, y10: null, all: null };
...
export function ownPerf(asset, pricer) {
  ...
  // "all" is anchored to the first purchase, not to the start of the quote series:
  // that keeps it meaningful instead of being a duplicate of the longest fixed period,
  // and it mirrors what "Tout" already means on the portfolio-wide chart
  const firstBuy = asset.buyLots?.[0]?.date ?? null;
  const all = firstBuy && Date.parse(firstBuy) >= Date.parse(firstDate) ? since(firstBuy) : null;
