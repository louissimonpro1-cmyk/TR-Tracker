...
export function buildIntradaySeries(ledger, market) {
  ...
  const inSession = (p) => p.entry.intra?.sessionDate === sessionDate && p.entry.intra.points.length;
  const baseline = items.reduce((s, { a, p }) => s + a.shares * (inSession(p) ? p.prevCloseEur : p.liveEur), 0);
  if (!sessionDate || baseline <= 0) return { sessionDate, sessionStart: null, points: [] };

  let prevSessionDate = null, sessionStartTs = Infinity;
  for (const { p } of items) {
    if (!inSession(p)) continue;
