...
import { resolveSymbol, dailyHistory, intradayHistory, hourlyHistory, monthlyHistory, getFx } from "./yahoo.mjs";
...
export async function collectMarket(ledger) {
  ...
  const entries = new Map(); // keyed by asset.key: the same ISIN can be held in two accounts
  await pmap(assets, 4, async (a) => {
    const symbol = await resolveSymbol(a).catch(() => null);
    const daily = symbol ? await dailyHistory(symbol).catch(() => null) : null;
    entries.set(a.key, { symbol, daily, intra: null });
  });
  await pmap(assets.filter((a) => a.isOpen && entries.get(a.key)?.daily), 4, async (a) => {
    const e = entries.get(a.key);
  ...
  await pmap(assets.filter((a) => recentlyActive(a) && entries.get(a.key)?.daily), 4, async (a) => {
    const e = entries.get(a.key);
    e.hourly = await hourlyHistory(e.symbol).catch(() => null);
  });
  // full listing history, only where it is actually shown: the "Tout" pill lives on the
  // open-position cards, so closed positions never pay for this extra request
  await pmap(assets.filter((a) => a.isOpen && entries.get(a.key)?.daily), 4, async (a) => {
    const e = entries.get(a.key);
    e.monthly = await monthlyHistory(e.symbol).catch(() => null);
  });
...
export function makePricer(asset, market) {
  const entry = market.entries.get(asset.key) || {};
  ...
  // monthly closes going back to the listing, for the "Tout" view. Dates predating the
  // FX series fall back to its oldest rate (fxRateAt): a 25-year curve is read for its
  // shape and order of magnitude, and no exchange rate that old is worth fetching.
  const monthlyEurAt = (date) => {
    if (!entry.monthly?.closes?.length) return null;
    const c = ffill(entry.monthly.closes, date);
    return c != null ? c / fxRateAt(date) : null;
  };
  ...
  // When there is no quote at all, the value shown is a transaction price frozen on
  // the day it was paid. Surfacing that date lets the interface say how stale it is,
  // instead of presenting a months-old price as if it were today's.
  // Computed as a max rather than the last element, so it holds even if the marks
  // ever stop arriving in chronological order.
  const priceDate = source === "transaction" && asset.priceMarks.length
    ? asset.priceMarks.reduce((acc, m) => (m.date > acc ? m.date : acc), asset.priceMarks[0].date)
    : null;

  return {
    currency, hasMarket, source, priceDate, liveEur, valLiveEur, prevCloseEur,
    eurAt, adjEurAt, intraEurAt, hourlyEurAt, monthlyEurAt,
    // divide a historical transaction price by this to compare it with today's price
    // (a share bought before a 10:1 split cost 10x what one share costs now)
    factorAfter, entry,
  };
}
...
export function buildDailyValues(ledger, market, opts = {}) {
  const { accountFilter = null, startDate = null } = opts;
  const start = startDate || ledger.firstDate;
  const end = todayParis();
  const dates = [];
  for (let t = Date.parse(start); t <= Date.parse(end); t += DAY_MS)
    dates.push(new Date(t).toISOString().slice(0, 10));

  const items = [...ledger.assets.values()]
    .filter((a) => !accountFilter || accountFilter.has(a.bucket))
    .map((a) => ({ a, p: makePricer(a, market) }));
...
export function perfSeries(dates, values, flowsByDate, startIdx, endIdx = dates.length - 1) {
...
export function buildHourlyValues(ledger, market, opts = {}) {
  const { accountFilter = null } = opts;
  const items = [...ledger.assets.values()]
    .filter((a) => !accountFilter || accountFilter.has(a.bucket))
    .map((a) => ({ a, p: makePricer(a, market) }));
...
export function buildIntradaySeries(ledger, market, opts = {}) {
  const { accountFilter = null } = opts;
  const open = [...ledger.assets.values()].filter((a) => a.isOpen && (!accountFilter || accountFilter.has(a.bucket)));
  const items = open.map((a) => ({ a, p: makePricer(a, market) }));

  // The reference session comes from listed assets only: crypto trades 24/7 and would
  // otherwise push the reference to a day on which no exchange has opened (a weekend),
  // flattening every stock. Crypto is always considered in session.
  let sessionDate = null;
  for (const { a, p } of items) {
    if (a.isCrypto) continue;
    const sd = p.entry.intra?.sessionDate;
    if (sd && (!sessionDate || sd > sessionDate)) sessionDate = sd;
  }
  if (!sessionDate) { // crypto-only portfolio
    for (const { p } of items) {
      const sd = p.entry.intra?.sessionDate;
      if (sd && (!sessionDate || sd > sessionDate)) sessionDate = sd;
    }
  }
  // baseline: previous close for assets trading in this session; assets whose market
  // has not opened yet (e.g. the US before 15:30 Paris) count at their static price
  // on BOTH sides, so their previous session's move never leaks into today's curve
  const inSession = (a, p) => !!p.entry.intra?.points?.length
    && (a.isCrypto || p.entry.intra.sessionDate === sessionDate);
  const baseline = items.reduce((s, { a, p }) => s + a.shares * (inSession(a, p) ? p.prevCloseEur : p.liveEur), 0);
  if (!sessionDate || baseline <= 0) return { sessionDate, sessionStart: null, points: [] };

  let prevSessionDate = null, sessionStartTs = Infinity;
  for (const { a, p } of items) {
    if (!inSession(a, p)) continue;
    const intra = p.entry.intra;
    if (intra.prevSessionDate && (!prevSessionDate || intra.prevSessionDate > prevSessionDate)) prevSessionDate = intra.prevSessionDate;
    if (intra.sessionStartTs != null && intra.sessionStartTs < sessionStartTs) sessionStartTs = intra.sessionStartTs;
  }
  if (!Number.isFinite(sessionStartTs)) sessionStartTs = null;

  // rolling window: start on the previous trading day at the current clock time
  // ("at 12h, show since 12h of the previous session"), clamped so the whole
  // current session always stays visible
  let windowStart = sessionStartTs ?? -Infinity;
  if (prevSessionDate && sessionStartTs != null) {
    const dayDiff = Math.max(1, Math.round((Date.parse(sessionDate) - Date.parse(prevSessionDate)) / 86400000));
    windowStart = Math.min(sessionStartTs, Date.now() / 1000 - dayDiff * 86400);
  }

  const tsSet = new Set();
  for (const { a, p } of items) {
    if (!inSession(a, p)) continue;
    for (const [ts] of p.entry.intra.points) if (ts >= windowStart) tsSet.add(ts);
  }
  const grid = [...tsSet].sort((x, y) => x - y);
  ...
  // anchor the curve at 0% only when no previous-session tail is displayed
  // (with a tail, the curve crosses 0% naturally at the previous close)
  if (points.length && (sessionStartTs == null || grid[0] >= sessionStartTs)) {
    points.unshift({ t: grid[0] * 1000 - 300000, pct: 0, value: baseline });
  }
  return {
    sessionDate,
    sessionStart: sessionStartTs != null ? sessionStartTs * 1000 : null,
    points,
  };
}

// ---- per-asset own performance over standard periods ----------------------
// A period whose start predates the quote series returns null, and the interface
// greys the matching pill out — better than silently showing a shorter period under
// a longer label (a stock listed 4 years ago has no honest 10-year figure).
const OWN_PERF_EMPTY = { d1: null, w1: null, m1: null, m6: null, y1: null, y3: null, y5: null, y10: null, since: null, all: null };

export function ownPerf(asset, pricer) {
  const { entry } = pricer;
  if (!pricer.hasMarket) return { ...OWN_PERF_EMPTY };
  ...
  // "since": from the first purchase of this holding — how the stock behaved over the
  // stretch that actually concerns the owner.
  const firstBuy = asset.buyLots?.[0]?.date ?? null;
  const sinceBuy = firstBuy && Date.parse(firstBuy) >= Date.parse(firstDate) ? since(firstBuy) : null;
  // "all": the entire listing history, off the monthly series (the daily one stops at
  // 10 years). Anchored on its first bar, so NVIDIA reads from 1999.
  const monthlyCloses = entry.monthly?.closes;
  let all = null;
  if (monthlyCloses?.length) {
    const base = pricer.monthlyEurAt(monthlyCloses[0][0]);
    if (base > 0) all = (live / base - 1) * 100;
  }
  ...
  return {
    ...
    y5: at(5 * 365 * DAY_MS, 40),
    y10: at(10 * 365 * DAY_MS, 60),
    since: sinceBuy,
    all,
  };
}
