...
import { buildLedger, mergeFlows } from "./ledger.mjs";
import {
...
export const PERF_RANGES = ["max", "10y", "8y", "5y", "3y", "1y", "6m", "1m", "1w", "1d", "custom"];
...
export async function getDashboard() {
  ...
  for (const { a, p } of items) {
    ...
    if (a.isOpen) {
      ...
      bucket.positions.push({
        ...
        account: a.bucket,
        qty: a.shares,
        priceEur: round3(p.liveEur), priceSource: p.source, priceDate: p.priceDate,
  ...
  const accounts = [...openByBucket.entries()]
    ...
    .map(([id, b]) => ({
      ...
      firstDate: ledger.firstDateByAccount.get(id) ?? null,
...
// ---- performance series ---------------------------------------------------
// opts.accounts: array of bucket ids to restrict the curve to (null/omitted = all).
// opts.from/opts.to (range === "custom" only): inclusive ISO date bounds; `from` is
// clamped to the selection's own first activity date, `to` to today.
const perfCache = new Map(); // cache key -> { at, data }
export async function getPerf(range, opts = {}) {
  if (!PERF_RANGES.includes(range)) throw Object.assign(new Error("range invalide"), { status: 400 });
  if (range === "custom" && (!ISO_DATE.test(opts.from || "") || !ISO_DATE.test(opts.to || ""))) {
    throw Object.assign(new Error("plage personnalisée : from/to requis (AAAA-MM-JJ)"), { status: 400 });
  }
  const accountFilter = opts.accounts?.length ? new Set(opts.accounts) : null;
  const cacheKey = `${range}|${accountFilter ? [...accountFilter].sort().join(",") : ""}|${opts.from || ""}|${opts.to || ""}`;
  ...
  // this selection's own start of activity: the earliest of the chosen accounts
  // (not the portfolio-wide firstDate), so a filtered/custom curve never implies
  // history that predates it
  let effectiveStart = ledger.firstDate;
  if (accountFilter) {
    let earliest = null;
    for (const id of accountFilter) {
      const d = ledger.firstDateByAccount.get(id);
      if (d && (!earliest || d < earliest)) earliest = d;
    }
    if (earliest) effectiveStart = earliest;
  }
  const flows = mergeFlows(ledger.flowsByAccount, accountFilter);

  let data;
  if (range === "custom") {
    const today = todayParis();
    const from = opts.from < effectiveStart ? effectiveStart : opts.from;
    const to = opts.to > today ? today : opts.to;
    if (from > to) throw Object.assign(new Error("plage de dates invalide"), { status: 400 });
    const { dates, values } = buildDailyValues(ledger, market, { accountFilter, startDate: effectiveStart });
    let startIdx = dates.findIndex((d) => d >= from);
    if (startIdx === -1) startIdx = 0;
    let endIdx = 0;
    for (let i = dates.length - 1; i >= 0; i--) { if (dates[i] <= to) { endIdx = i; break; } }
    data = pack(perfSeries(dates, values, flows, startIdx, endIdx));
  } else if (range === "1d") {
    const { sessionDate, sessionStart, points } = buildIntradaySeries(ledger, market, { accountFilter });
    data = { ...pack(points, sessionDate), sessionStart };
  } else if (range === "1w" || range === "1m") {
    // hourly granularity: one point per market hour (1w), ~4 per day (1m)
    const { grid, dates, values } = buildHourlyValues(ledger, market, { accountFilter });
    const startTs = Date.now() - (range === "1w" ? 7 : 30) * 86400000;
    let startIdx = grid.findIndex((t) => t >= startTs);
    if (startIdx === -1) startIdx = 0;
    if (grid.length - startIdx >= 10) {
      let points = perfSeriesHourly(grid, dates, values, flows, startIdx);
  ...
  if (!data) { // daily fallback (also covers hourly data being unavailable)
    ...
    data = pack(perfSeries(dates, values, flows, startIdx));
  }
  perfCache.set(cacheKey, { at: Date.now(), data });
  return data;
}
...
export async function getAssetSeries(key) {
  ...
  const hourly = (p.entry.hourly?.points || [])
    ...
    .filter((x) => x[1] != null);
  // monthly closes back to the listing, for the "Tout" view
  const monthly = (p.entry.monthly?.closes || [])
    .map(([d]) => [d, round3(p.monthlyEurAt(d))])
    .filter((x) => x[1] != null);
  // firstBuyDate anchors the "Depuis achat" view (see ownPerf in portfolio.mjs)
  const data = {
    key, isin: asset.isin,
    firstBuyDate: asset.buyLots?.[0]?.date ?? null,
    daily, hourly, monthly, intraday,
  };
