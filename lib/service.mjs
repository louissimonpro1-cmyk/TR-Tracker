...
export async function getAssetSeries(key) {
  ...
  const hourly = (p.entry.hourly?.points || [])
    ...
    .filter((x) => x[1] != null);
  // firstBuyDate anchors the "Tout" pill, which runs from the first purchase rather
  // than from the start of the quote series (see ownPerf in portfolio.mjs)
  const data = { key, isin: asset.isin, firstBuyDate: asset.buyLots?.[0]?.date ?? null, daily, hourly, intraday };
