...
export async function resolveSymbol(asset) {
  ...
  return resolveIsin(asset.isin, asset.name);
}

// Yahoo's ISIN lookup answers for most instruments, but not all — and an ISIN it
// cannot answer leaves the holding with no series whatsoever: no chart, and a price
// frozen at the last transaction. Searching the name recovers the primary listing in
// most cases. It is only a fallback: a name can match a neighbouring share class or
// a foreign listing, which the transaction-price check in portfolio.mjs then catches
// (it compares the series to the prices actually paid and rescales past 4 %).
const SEARCHABLE_TYPES = new Set(["EQUITY", "ETF", "MUTUALFUND"]);

async function searchByName(name) {
  // statements suffix the issuer with a share class ("Alphabet (A)"), a form Yahoo's
  // search does not understand at all — it returns zero quotes for it, so strip it.
  const q = String(name || "").replace(/\([^)]*\)/g, " ").replace(/\s+/g, " ").trim();
  if (q.length < 3) return null;
  const j = await yjson(`https://query2.finance.yahoo.com/v1/finance/search?q=${encodeURIComponent(q)}&quotesCount=10&newsCount=0`);
  const hits = (j?.quotes || []).filter((x) => x.symbol && SEARCHABLE_TYPES.has(x.quoteType));
...
export async function resolveIsin(isin, name = null) {
  const pinned = SYMBOL_OVERRIDES[isin];
  ...
  try {
    ...
    if (!q && name) {
      q = await searchByName(name);
      if (q) {
        via = "name";
        console.warn(`[cotation] ${name} (${isin}) : ISIN introuvable chez Yahoo, replié sur ${q.symbol} trouvé par nom`);
      }
    }
  } catch { return e?.symbol ?? null; } // network trouble: keep whatever we knew
  if (!q) console.warn(`[cotation] ${name || isin} (${isin}) : aucune cotation trouvée, valorisation au dernier prix de transaction`);
  symbols[isin] = { symbol: q?.symbol ?? null, name: q?.longname ?? q?.shortname ?? null, at: Date.now(), via };
...
// one month of hourly bars, for the 1-week and 1-month views
const memHourly = new Map();
export async function hourlyHistory(symbol) {
  const hit = memHourly.get(symbol);
  if (hit && Date.now() - hit.at < DAILY_TTL) return hit.data;
  const file = path.join(HIST_DIR, `${safeName(symbol)}-1h.json`);
  if (!hit) {
    try {
      const disk = JSON.parse(await fs.readFile(file, "utf8"));
  ...
  try {
    const j = await yjson(`https://query1.finance.yahoo.com/v8/finance/chart/${encodeURIComponent(symbol)}?range=1mo&interval=1h&includePrePost=false`);
    const c = parseChart(j);
    if (!c) throw new Error(`no hourly data for ${symbol}`);
    const data = {
      currency: c.currency,
